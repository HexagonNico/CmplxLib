...
class Vec2cSuite extends AnyFunSuite {
  ...
  test("Vector multiplied by a complex number") {
    ...
    assert(res == Vec2c(Complex(-0.5, 4.0), Complex(1.25, 3.0)))
  }
  ...
  test("Vector divided by a complex number") {
    ...
    assert(res == Vec2c(Complex(3.0 / 2.0, 0.5), Complex(1.25, -0.25)))
  }
  ...
  test("Multiplicative inverse of a vector") {
    val vec = Vec2c(Complex(2.0, 4.0), Complex(8.0, 6.0))
    assert(vec.inverse == Vec2c(Complex(0.1, -0.2), Complex(0.08, -0.06)))
  }
